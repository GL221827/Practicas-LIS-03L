...
class empleado {
    ...
    private $prestamo; // Nueva propiedad para el descuento de préstamo
    ...
    function __destruct() {
        ...
        $backlink = "<a class=\"a-btn\" href=\"Ej3_sueldoneto.php\">";
    ...
    function obtenerSalarioNeto($nombre, $apellido, $salario, $horasextras, $pagoxhoraextra = 0.0, $prestamo = 0.0) {
        ...
        $salarioBase = $salario + $this->pagoxhoraextra;
        $this->isss = $salarioBase * self::descISSS;
        $this->afp = $salarioBase * self::descAFP;

        // Cálculo de Renta
        $salarioConDescuentos = $salarioBase - ($this->isss + $this->afp);
        ...
        // Cálculo de sueldo líquido incluyendo préstamo si aplica
        $this->sueldoLiquido = $salarioBase - ($this->isss + $this->afp + $this->renta + $this->prestamo);
        
        $this->imprimirBoletaPago();
    }

    // Método para imprimir la boleta de pago
    function imprimirBoletaPago() {
        ...
        $tabla .= "<tr><td>Descuento ISSS: </td><td>$ " . number_format($this->isss, 2, '.', ',') . "</td></tr>";
        ...
        // Mostrar descuento por préstamo solo si existe
        if ($this->prestamo > 0) {
            $tabla .= "<tr><td>Descuento por préstamo: </td><td>$ " . number_format($this->prestamo, 2, '.', ',') . "</td></tr>";
        }
